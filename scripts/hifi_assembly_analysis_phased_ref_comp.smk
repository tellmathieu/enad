...
rule all:
  input:
    ...
    buscoPDF = os.path.join(mainDir, 'buscoPrimary.pdf')
...
echo "Strain,Complete_single_copy,Complete_duplicated,Fragmented,Missing" > {output.buscoCSV}
    ...
    cat {input.outputSummary} | grep "(S)" | awk -v strain="{wildcards.sample}" '{{print strain","$1}}' > {params.buscoDir}/complete_single_primary.txt

    cat {input.outputSummary} | grep "(D)" | awk '{{print $1}}' > {params.buscoDir}/complete_duplicated_primary.txt

    cat {input.outputSummary} | grep "(F)" | awk '{{print $1}}' > {params.buscoDir}/fragmented_primary.txt

    cat {input.outputSummary} | grep "(M)" | awk '{{print $1}}' > {params.buscoDir}/missing_primary.txt

    paste -d "," {params.buscoDir}/complete_single_primary.txt {params.buscoDir}/complete_duplicated_primary.txt {params.buscoDir}/fragmented_primary.txt {params.buscoDir}/missing_primary.txt >> {output.buscoCSV}

    rm {params.buscoDir}/complete_single_primary.txt {params.buscoDir}/complete_duplicated_primary.txt {params.buscoDir}/fragmented_primary.txt {params.buscoDir}/missing_primary.txt
'''

rule combineBUSCOPrimary:
  input:
    buscoCSV = expand(os.path.join(mainDir, '{sample}', 'busco.csv'), sample = SAMPLES)
  output:
    combinedBuscoCSV = os.path.join(mainDir, 'combinedBuscoPrimary.csv')
  shell: '''
    echo "Strain,Complete_single_copy,Complete_duplicated,Fragmented,Missing" > {output.combinedBuscoCSV}
    cat {input.buscoCSV} | sort | uniq -u >> {output.combinedBuscoCSV}
  '''

rule buscoRscriptPrimary:
  input:
    combinedBuscoCSV = os.path.join(mainDir, 'combinedBuscoPrimary.csv')
  params:
    buscoRscript = os.path.join(mainDir, 'scripts', 'busco.R')
...
rule plotStructuralRearrangementCombined:
  input:
    ...
    genomeTxtFile = os.path.join(mainDir, 'syri_genomes.txt'),
    svBed = os.path.join(mainDir, 'resources', 'candidates_manual.bed'),
  params:
...
rule alignToFirstControl:
  ...
  output:
    compareToControl = os.path.join(mainDir, 'sniffles', '{comp_sample}.{phases}_comptoref.sam')
  shell: '''
...
rule alignToFirstControlPrimary:
  ...
  output:
    compareToControl = os.path.join(mainDir, 'snifflesPrimary', '{sample}.primary_comptoref.sam')
  shell: '''
    mkdir -p {params.snifflesDir}
...
rule convertSamToBam:
  input:
     os.path.join(mainDir, 'sniffles', '{comp_sample}.{phases}_comptoref.sam')
  output:
    os.path.join(mainDir, 'sniffles', '{comp_sample}.{phases}_comptoref.bam')
  shell: '''
...
rule sortBam:
  input:
    os.path.join(mainDir, 'sniffles', '{comp_sample}.{phases}_comptoref.bam')
  output:
    os.path.join(mainDir, 'sniffles', '{comp_sample}.{phases}_comptoref_sorted.bam')
  shell: '''
...
rule indexBam:
  input:
    os.path.join(mainDir, 'sniffles', '{comp_sample}.{phases}_comptoref_sorted.bam')
  output:
    os.path.join(mainDir, 'sniffles', '{comp_sample}.{phases}_comptoref_sorted.bam.bai')
  shell: '''
...
rule findSVs:
  resources:
    mem_mb=50000
  input:
    compareToControlBam = os.path.join(mainDir, 'sniffles', '{comp_sample}.{phases}_comptoref_sorted.bam'),
    bai = os.path.join(mainDir, 'sniffles', '{comp_sample}.{phases}_comptoref_sorted.bam.bai')
  output:
...
rule convertSamToBamPrimary:
  input:
     os.path.join(mainDir, 'snifflesPrimary', '{comp_sample}.primary_comptoref.sam')
  output:
    os.path.join(mainDir, 'snifflesPrimary', '{comp_sample}.primary_comptoref.bam')
  shell: '''
    samtools view -S -b {input} > {output}
    rm {input}
  '''
...
rule sortBamPrimary:
  input:
    os.path.join(mainDir, 'snifflesPrimary', '{comp_sample}.primary_comptoref.bam')
  output:
    os.path.join(mainDir, 'snifflesPrimary', '{comp_sample}.primary_comptoref_sorted.bam')
  shell: '''
    samtools sort -o {output} {input}
    rm {input}
  '''
...
rule indexBamPrimary:
  input:
    os.path.join(mainDir, 'snifflesPrimary', '{comp_sample}.primary_comptoref_sorted.bam')
  output:
    os.path.join(mainDir, 'snifflesPrimary', '{comp_sample}.primary_comptoref_sorted.bam.bai')
  shell: '''
    samtools index -b {input}
  '''
...
rule findSVsPrimary:
  resources:
    mem_mb=50000
  input:
    compareToControlBam = os.path.join(mainDir, 'snifflesPrimary', '{comp_sample}.primary_comptoref_sorted.bam'),
    bai = os.path.join(mainDir, 'snifflesPrimary', '{comp_sample}.primary_comptoref_sorted.bam.bai')
  output:
