...
control = os.path.join(mainDir, order_comps[0], order_comps[0] + '.scaffold_chr_only.fa')
...
rule all:
  input:
    os.path.join(mainDir, 'sniffles', 'phasedSVs1.vcf'),
    expand(os.path.join(mainDir, 'chainAlign', '{comp1}.{phases}-{comp2}.{phases}comparison.sam'), zip, comp1 = first_comp, comp2 = second_comp, phases = PHASES),
    expand(os.path.join(mainDir, '{sample}', '{sample}.scaffold_chr_only.fa'), sample = order_comps),
    os.path.join(mainDir, 'snifflesPlotPrimary', 'heatmap.jpg'),
    os.path.join(mainDir, 'snifflesPlotPrimary', 'sample_upset.jpg'),
    os.path.join(mainDir, 'snifflesPlotPrimary', 'sv_site_per_genome.jpg'),
    os.path.join(mainDir, 'snifflesPlotPrimary', 'variant_count.jpg')
...
rule alignToChainOfGenomes:
  resources:
    mem_mb=50000 
  input:
    comp1 = os.path.join(mainDir, '{comp1}.{phases}', '{comp1}.{phases}_scaffold_chr_only.fa'),
    comp2 = os.path.join(mainDir, '{comp2}.{phases}', '{comp2}.{phases}_scaffold_chr_only.fa')
  params:
    chainDir = os.path.join(mainDir, 'chainAlign')
  output:
    compareSam = os.path.join(mainDir, 'chainAlign', '{comp1}.{phases}-{comp2}.{phases}comparison.sam')
  shell: '''
...
rule syntenyAnalysisForCombined:
  ...
  input:
    ...
    compareSam = os.path.join(mainDir, 'chainAlign', '{comp1}.{phases}-{comp2}.{phases}comparison.sam')
  params:
    outputDir = os.path.join(mainDir, 'chainAlign', '{comp1}.{phases}-{comp2}.{phases}syri')
  output:
    outFile = os.path.join(mainDir, 'chainAlign', '{comp1}.{phases}-{comp2}.{phases}syri', '{comp1}.{phases}-{comp2}.{phases}syri.out')
  shell: '''
    ...
    syri \
      ...
      -q {input.comp2} \
      -k -F S \
      --prefix {wildcards.comp1}.{wildcards.phases}-{wildcards.comp2}.{wildcards.phases}\
...
rule plotStructuralRearrangementCombined:
  input:
    first_comphap1_File = os.path.join(mainDir, order_comps[0] + '.{phases}', PHASES[0] + '.syri', 'reference_' + order_comps[0] + '.' + PHASES[0] + 'syri.out'),
    first_comphap2_File = os.path.join(mainDir, order_comps[0] + '.{phases}', PHASES[1] + '.syri', 'reference_' + order_comps[0] + '.' + PHASES[1] + 'syri.out'),
    outFileChain = expand(os.path.join(mainDir, 'chainAlign', '{comp1}.{phases}-{comp2}.{phases}syri', '{comp1}.{phases}-{comp2}.{phases}syri.out'), zip, comp1 = first_comp, comp2 = second_comp, phases = PHASES),
    genomeTxtFile = os.path.join(mainDir, 'syri_genomes.txt')
  params:
    plotsrGraphs = os.path.join(mainDir, 'plotsrGraphsPhased')
  output:
    plot = os.path.join(mainDir, 'plotsrGraphsPhased', 'syri_{chr}_combined_output_plot.png')
  shell: '''
    mkdir -p {params.plotsrGraphs}

    COMPS=({input.outFileChain})
    SR=()
    SR+=("--sr {input.first_comphap1_File} --sr {input.first_comphap2_File} ")
    for sample in ${{COMPS[@]}}
      do
        SR+=("--sr $sample ")
      done
...
rule plotStructuralRearrangementCombinedWithPOI:
  resources:
    mem_mb=50000 
  input:
    first_comphap1_File = os.path.join(mainDir, order_comps[0] + '.{phases}', '{phases}.syri', 'reference_' + order_comps[0] + '.' + PHASES[0] + 'syri.out'),
    first_comphap2_File = os.path.join(mainDir, order_comps[0] + '.{phases}', '{phases}.syri', 'reference_' + order_comps[0] + '.' + PHASES[1] + 'syri.out'),
    outFileChain = expand(os.path.join(mainDir, 'chainAlign', '{comp1}.{phases}-{comp2}.{phases}syri', '{comp1}.{phases}-{comp2}.{phases}syri.out'), zip, comp1 = first_comp, comp2 = second_comp, phases= PHASES),
    genomeTxtFile = os.path.join(mainDir, 'syri_genomes.txt'),
    ...
    chrLen = os.path.join(mainDir, 'resources', 'chrFinal.len')
  params:
    plotsrGraphsPOI = os.path.join(mainDir, 'plotsrGraphsPOIPhased')
  output:
    plot = os.path.join(mainDir, 'plotsrGraphsPOIPhased', 'syri_{chr}_combined_output_plot.png')
  shell: '''
    mkdir -p {params.plotsrGraphsPOI}

    COMPS=({input.outFileChain})
    SR{wildcards.chr}=()
    SR{wildcards.chr}+=("--sr {input.first_comphap1_File} --sr {input.first_comphap2_File} ")
    for sample in ${{COMPS[@]}}
      do
...
rule compareSVs:
  resources:
    mem_mb=50000
  input:
    snf = expand(os.path.join(mainDir, 'sniffles', '{comp_sample}.{phases}.snf'), comp_sample = order_comps, phases = PHASES)
  output:
...
rule compareSVsPrimary:
  resources:
    mem_mb=50000
  input:
    snf = expand(os.path.join(mainDir, 'snifflesPrimary', '{comp_sample}.primary.snf'), comp_sample = order_comps)
  output:
...
rule plotSVs:
  ...
  output:
    os.path.join(mainDir, 'snifflesPlot', 'heatmap.jpg'),
    os.path.join(mainDir, 'snifflesPlot', 'sample_upset.jpg'),
    os.path.join(mainDir, 'snifflesPlot', 'sv_site_per_genome.jpg'),
    os.path.join(mainDir, 'snifflesPlot', 'variant_count.jpg')
  shell: '''
...
rule createCommonSvsPrimaryBed:
  input:
    multiVCF = os.path.join(mainDir, 'snifflesPrimary', 'primarySVs1.vcf')
  output:
    primaryVCFBED = os.path.join(mainDir, 'snifflesPrimary', 'primarySVsCommon.bed')
  shell: '''
