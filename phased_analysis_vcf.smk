...
rule all:
  input:
    ...
    expand(os.path.join(mainDir, 'results', '{hap}_{phases}_vcf_gff_intersect.txt'), zip, hap = hap, phases = PHASES)
...
rule cleanAndFilterVCF:
  ...
  shell: '''
    sed "s/GT:GQ:DR:DV:PS:ID/GT:GQ:DR:DV:ID/g" {input.multiVCF} > temp_cleaned_{wildcards.hap}.vcf
    bgzip temp_cleaned_{wildcards.hap}.vcf -k
    bcftools index temp_cleaned_{wildcards.hap}.vcf.gz
    bcftools view -i '(GT[0]=="0/0" && GT[1]=="0/0" && GT[2]=="0/0") && ((GT[3]=="0/1" || GT[3]=="1/0" || GT[3]=="1/1") && (GT[4]=="0/1" || GT[4]=="1/0" || GT[4]=="1/1") && (GT[5]=="0/1" || GT[5]=="1/0" || GT[5]=="1/1"))' temp_cleaned_{wildcards.hap}.vcf.gz -Ov -o {output.filteredVCF}
    rm temp_cleaned_{wildcards.hap}*
  '''
...
rule intersectGFFandVCF:
  input:
    filteredVCF = os.path.join(mainDir, 'sniffles', '{hap}_filtered.vcf'),
    gff= os.path.join(mainDir, 'resources', order_comps[0] + '_{phases}.gff')
  params:
    os.path.join(mainDir, 'results')
  output:
    intersect = os.path.join(mainDir, 'results', '{hap}_{phases}_vcf_gff_intersect.txt')
  shell: '''
    mkdir -p {params}
